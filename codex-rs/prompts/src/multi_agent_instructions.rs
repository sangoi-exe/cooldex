...
pub enum MultiAgentRoleInstructions {
    ...
    /// Durable full-history hint text restored without recomposing current policy.
    Captured { text: String, marked: bool },
}

impl MultiAgentRoleInstructions {
    pub fn unmarked(text: impl Into<String>) -> Self {
        Self::Captured {
            text: text.into(),
            marked: false,
        }
    ...
    pub fn from_agent_usage_hint_instructions(
        AgentUsageHintInstructions { text, marked }: AgentUsageHintInstructions,
    ) -> Self {
        Self::Captured { text, marked }
    }

    pub fn into_agent_usage_hint_instructions(self) -> AgentUsageHintInstructions {
        match self {
            Self::Configured(text) => AgentUsageHintInstructions {
                text,
                marked: false,
            },
            Self::Captured { text, marked } => AgentUsageHintInstructions { text, marked },
            instructions @ Self::Composed { marked, .. } => AgentUsageHintInstructions {
...
impl ContextualUserFragment for MultiAgentRoleInstructions {
    ...
    fn markers(&self) -> (&'static str, &'static str) {
        match self {
            ...
            Self::Configured(_)
            | Self::Composed { marked: false, .. }
            | Self::Captured { marked: false, .. } => ("", ""),
        }
    }
    ...
    fn body(&self) -> String {
        match self {
            Self::Configured(text) | Self::Captured { text, .. } => text.clone(),
            Self::Composed {
                base,
