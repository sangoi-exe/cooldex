...
// Merge-safety anchor: verified apply-patch parsing in this workspace must keep host-local patch
// verification and executor-fs-backed path resolution aligned with the split apply-patch owners in
// `lib.rs`; merges must not collapse them back together.
