...
fn spawn_config_spans(
    ...
    // Merge-safety anchor: spawn rows in the TUI app-server surface must render the effective
    // child profile/model/reasoning from `CollabAgentSpawnEndEvent`, not stale begin-request
    // cache data.
...
// Merge-safety anchor: wait titles and details in the TUI app-server surface must stay aligned
// with collab wait metadata so any_final/all_final and timeout returns remain visible.
fn wait_begin_title(
