...
// Merge-safety anchor: InstanceChild spawning stays in the dedicated supervisor while generic
// connections remain upstream-owned and can never turn into a child spawn.
...
async fn start_app_server(
    ...
    if matches!(target, AppServerTarget::InstanceChild) {
        #[cfg(target_os = "linux")]
        {
            ...
            .await?;
            return Ok(AppServerSession::new_instance_child(
                started.client,
                started.supervisor,
            ));
        }
...
fn app_server_target_for_launch(
    ...
    Ok(match explicit_remote_endpoint {
        ...
        // Merge-safety anchor: an explicit instance-owned mode bypasses daemon discovery and the
        // child can never be selected by a generic daemon fallback.
...
async fn run_ratatui_app(
    ...
    let app_server_session = match startup_app_server {
        Ok(Ok(app_server)) => app_server.with_local_codex_home(&initial_config.codex_home),
        Ok(Err(err)) => {
            terminal_restore_guard.restore_silently();
            session_log::log_session_end();
            return Err(err);
    ...
    if !uses_remote_workspace || remote_cwd_override.is_some() {
        ...
        if consent.directory_trust_persisted && !uses_remote_workspace {
                ...
                && matches!(app_server_target, AppServerTarget::Embedded)
            {
                ...
                let app_server_session = start_app_server(
                    &mut app_server_target,
                    arg0_paths.clone(),
                    config.clone(),
                    raw_config_overrides.clone(),
...
"#
    ...
    #[test]
    fn app_server_target_for_launch_selects_instance_child_without_remote() -> color_eyre::Result<()>
    {
        let target = app_server_target_for_launch(
            /*explicit_remote_endpoint*/ None,
