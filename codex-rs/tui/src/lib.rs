...
pub async fn run_main(
    ...
    // Discover configured environments before loading config so a remote default can
    // suppress the local cwd. Build the manager only after the final HTTP policy is known.
