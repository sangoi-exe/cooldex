// Merge-safety anchor: status snapshot/tests are followers of the single `/status` rendering owner;
// merges must keep missing/unavailable/stale expectations aligned with `status/card.rs` and
// `status/rate_limits.rs` instead of preserving stale text only in tests.
...
#[tokio::test]
async fn status_snapshot_includes_auth_session_id() {
    let temp_home = TempDir::new().expect("temp home");
    let mut config = test_config(&temp_home).await;
    config.model = Some("gpt-5.1-codex-max".to_string());
    config.model_provider_id = "openai".to_string();
    ...
    let composite = new_status_output_with_rate_limits(
        ...
        Some(&token_info),
        &usage,
        Some("019d943e-a836-7071-9c92-8fb11079df45"),
        &None,
        /*thread_name*/ None,
        /*forked_from*/ None,
        &[],
