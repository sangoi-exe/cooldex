...
pub(crate) fn server_version_notice_key(
    ...
    // Merge-safety anchor: InstanceChild shares Embedded's local status identity and has no
...
mod tests {
    ...
    fn remote_connection_status_value_formats_display_value() -> color_eyre::Result<()> {
        ...
        assert_eq!(
            remote_connection_status_value(&embedded, Some("1.2.3")),
            None
        );
        assert_eq!(
            remote_connection_status_value(&AppServerTarget::InstanceChild, Some("1.2.3")),
            remote_connection_status_value(&embedded, Some("1.2.3"))
        );
    ...
    fn notice_key_identifies_socket_and_server_home_across_connection_modes() -> anyhow::Result<()>
    {
        ...
        assert_eq!(
            key(&AppServerTarget::InstanceChild, Some("/a")),
            key(&AppServerTarget::Embedded, Some("/a"))
        );
