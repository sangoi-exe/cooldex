...
pub(super) fn start_headless_chatgpt_login(widget: &mut AuthModeWidget) {
    // Merge-safety anchor: device-code onboarding must stay aligned with the
    // app-server-backed ChatGPT login/account surfaces so `/accounts` and TUI
    // sign-in do not drift onto incompatible transport contracts.
