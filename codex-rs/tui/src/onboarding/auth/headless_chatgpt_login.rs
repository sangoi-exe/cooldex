...
pub(super) fn start_headless_chatgpt_login(widget: &mut AuthModeWidget) {
    // Merge-safety anchor: device-code fallback and forced-workspace handling must stay aligned
    // with the app-server-backed ChatGPT login flow owned by onboarding auth.
...
mod tests {
    ...
    fn set_device_code_error_for_active_attempt_updates_only_when_active() {
        ...
        assert_eq!(error.read().unwrap().as_deref(), Some("device code failed"));
        ...
        *error.write().unwrap() = None;
        ...
        assert_eq!(error.read().unwrap().as_deref(), None);
    }
