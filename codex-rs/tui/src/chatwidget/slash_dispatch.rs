    ...
    pub(super) fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            SlashCommand::Sanitize => {
                self.clear_prompt_gc_private_context_usage_info();
                self.app_event_tx
                    .send(AppEvent::CodexOp(AppCommand::from(Op::Sanitize)));
            }
            SlashCommand::Review => {
            ...
            SlashCommand::Debug => {
                // Merge-safety anchor: `/debug` reads the cache fed by `EventMsg::RawResponseItem`
                // and reset on rollback, so availability must follow that exact lifecycle.
