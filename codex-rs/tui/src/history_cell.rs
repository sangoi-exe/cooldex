...
// Merge-safety anchor: direct-runtime `/mcp` rendering still owns the
// `McpListToolsResponseEvent` snapshot path in this workspace; keep this
// function live and aligned with the app-server status renderer instead of
// silently collapsing both onto one partial data source during merges.
pub(crate) fn new_mcp_tools_output(
    ...
    let mut servers: Vec<_> = effective_servers.into_iter().collect();
    ...
    for (server, cfg) in servers {
        ...
        if !cfg.enabled {
            ...
            if let Some(reason) = cfg.disabled_reason.as_ref().map(ToString::to_string) {
                lines.push(vec!["    • Reason: ".into(), Span::from(reason).dim()].into());
            }
