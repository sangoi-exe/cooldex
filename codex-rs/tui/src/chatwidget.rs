...
impl RestoredPendingInputState {
    ...
    fn restored_composer_state(&self) -> Option<ThreadComposerState> {
        let mut messages: Vec<UserMessage> = self.rejected_steers_queue.iter().cloned().collect();
        messages.extend(
            self.pending_steers
                .iter()
...
impl ChatWidget {
    ...
    fn dispatch_command(&mut self, cmd: SlashCommand) {
        ...
        match cmd {
            ...
            // Merge-safety anchor: slash-command dispatch must keep `/subagents`
            // and `/permissions` as the only active legacy canon here.
