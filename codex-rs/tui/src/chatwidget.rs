...
fn app_server_collab_receiver_identity(
    ...
    if let Some(agent_state) = agents_states.get(receiver_thread_id)
        && (agent_state.agent_nickname.is_some()
            || agent_state.agent_role.is_some()
            || agent_state.task_name.is_some())
    {
        return (
            agent_state.agent_nickname.clone(),
            agent_state.agent_role.clone(),
            agent_state.task_name.clone(),
        );
    }
    ...
    receiver_agents
        ...
        .or_else(|| {
            app_server_collab_thread_id_to_core(receiver_thread_id).and_then(|thread_id| {
                collab_agent_metadata
                    .get(&thread_id)
                    .cloned()
                    .map(|metadata| (metadata.agent_nickname, metadata.agent_role, None))
            })
        })
        .unwrap_or_default()
}
...
fn app_server_collab_agent_statuses_to_core(
    ...
    for receiver_thread_id in receiver_thread_ids {
        ...
        let (agent_nickname, agent_role, task_name) = app_server_collab_receiver_identity(
            receiver_thread_id,
            receiver_agents,
            agents_states,
            collab_agent_metadata,
...
impl RestoredPendingInputState {
    ...
    fn restored_composer_state(&self) -> Option<ThreadComposerState> {
        ...
        for (idx, message) in self.rejected_steers_queue.iter().cloned().enumerate() {
            let history_record = self
                .rejected_steer_history_records
                .get(idx)
                .cloned()
                .unwrap_or(UserMessageHistoryRecord::UserMessageText);
            messages.push(user_message_for_restore(message, &history_record));
        }
        messages.extend(self.pending_steers.iter().map(|pending| {
            user_message_for_restore(pending.user_message.clone(), &pending.history_record)
        }));
        for (idx, message) in self.queued_user_messages.iter().cloned().enumerate() {
            let history_record = self
...
fn app_server_collab_interaction_tool_to_core(
    tool: &CollabAgentTool,
) -> codex_protocol::protocol::CollabAgentInteractionTool {
    match tool {
        ...
        other => panic!("unexpected collab interaction tool: {other:?}"),
    }
}
...
impl ChatWidget {
    ...
    pub(crate) fn set_pending_resume_turns(&mut self, turns: Option<Vec<Turn>>) {
        self.pending_resume_turns = turns;
    }
    ...
    fn drain_pending_input_for_restore(&mut self) -> Option<RestoredPendingInputState> {
        ...
        Some(RestoredPendingInputState {
            rejected_steers_queue: self.rejected_steers_queue.drain(..).collect(),
            rejected_steer_history_records: self.rejected_steer_history_records.drain(..).collect(),
    ...
    fn on_collab_agent_tool_call(&mut self, item: ThreadItem) {
        ...
        match tool {
            CollabAgentTool::SpawnAgent => {
                if matches!(status, CollabAgentToolCallStatus::Completed) {
                    ...
                    let (
                        ...
                        let (new_agent_nickname, new_agent_role, new_agent_task_name) =
                            first_receiver
                                .as_ref()
                                .map(|thread_id| {
            ...
            interaction_tool @ (CollabAgentTool::SendInput
            | CollabAgentTool::SendMessage
            | CollabAgentTool::FollowupTask) => {
                if let Some(receiver_thread_id) = first_receiver
                    && !matches!(status, CollabAgentToolCallStatus::InProgress)
                {
                    let (receiver_agent_nickname, receiver_agent_role, receiver_agent_task_name) =
                        app_server_collab_receiver_identity(
                    ...
                    self.on_collab_event(multi_agents::interaction_end(
                        codex_protocol::protocol::CollabAgentInteractionEndEvent {
                            call_id: id,
                            tool: app_server_collab_interaction_tool_to_core(&interaction_tool),
                            sender_thread_id,
                            receiver_thread_id,
            ...
            CollabAgentTool::Wait => {
                ...
                if matches!(status, CollabAgentToolCallStatus::InProgress) {
                    let mut receiver_agents =
                        app_server_collab_receiver_agents_to_core(&receiver_agents);
    ...
    pub(crate) fn new_from_existing(
        ...
        conversation: std::sync::Arc<crate::legacy_core::CodexThread>,
        ...
        let current_cwd = Some(session_configured.cwd.to_path_buf());
        let codex_op_target = CodexOpTarget::Direct(spawn_agent_from_existing(
            conversation,
            session_configured,
            app_event_tx.clone(),
        ));
    ...
    fn handle_thread_item(
        ...
        match item {
            ...
            ThreadItem::FileChange {
                ...
                let core_changes = file_update_changes_to_core(changes.clone());
            ...
            ThreadItem::WebSearch { id, query, action } => {
                if !from_replay || Self::web_search_renders_on_resume(&query, &action) {
            ...
            ThreadItem::ImageGeneration {
                ...
                if !from_replay
                    || Self::image_generation_renders_on_resume(
                        &status,
                        &revised_prompt,
                        &result,
                        &saved_path,
        ...
        match request {
            ...
            ServerRequest::DynamicToolCall { .. } => {
                if replay_kind.is_none() {
                    self.add_error_message(
                        "Dynamic tool requests must be resolved before they reach the chat widget."
                            .to_string(),
                    );
                }
            }
    ...
    pub(crate) fn handle_server_notification(
        ...
        match notification {
            ...
            ServerNotification::TurnStarted(notification) => {
                ...
                self.last_non_retry_error = None;
                self.apply_turn_started_context_window(self.status_line_context_window_size());
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            // Merge-safety anchor: the widget must render spawn rows from the effective end-event
            // payload; do not restore begin-event request caching here.
    ...
    pub(crate) fn maybe_send_restored_follow_up(&mut self) {
        ...
        let pending_steers: Vec<(UserMessage, UserMessageHistoryRecord)> = self
            .pending_steers
            .drain(..)
            .map(|pending| (pending.user_message, pending.history_record))
            .collect();
        ...
        for preset in presets.into_iter() {
            ...
            let requires_confirmation = preset.id == "full-access"
                && !self
                    ...
                    .unwrap_or(false);
            let uses_workspace_write_on_request = preset.id == "auto";
            ...
            if preset.id == "auto" {
                ...
                {
                    let guardian_preset = guardian_approval_preset();
                    items.push(SelectionItem {
                        name: guardian_preset.label.to_string(),
                        description: Some(guardian_preset.description.to_string()),
                        ...
                        actions: Self::approval_preset_actions(
                            preset.approval,
                            preset.permission_profile.clone(),
                            guardian_preset.label.to_string(),
    ...
    fn rate_limit_window_reset_at_local(window: &RateLimitWindow) -> Option<DateTime<Local>> {
    ...
    fn rate_limit_secondary_label(snapshot: Option<&RateLimitSnapshot>) -> String {
        snapshot
            .and_then(|snapshot| {
                snapshot
                    .secondary
    ...
    pub(crate) fn open_auto_review_denials_popup(&mut self) {
        ...
        items.extend(self.recent_auto_review_denials.entries().map(|event| {
            ...
            let rationale = event
                .rationale
                .as_deref()
                .unwrap_or("Auto-review denied this action.");
            SelectionItem {
                name: summary.clone(),
                description: Some(rationale.to_string()),
    ...
    pub(crate) fn open_world_writable_warning_confirmation(
        ...
        let (approval, approvals_reviewer, permission_profile) = match &preset {
            Some(p) => (
                Some(p.approval),
        ...
        // Suppress the immediate re-scan only when a preset will be applied
        // (for example via `/permissions`) to avoid duplicate warnings from
        // the ensuing policy change.
        if preset.is_some() {
            accept_actions.push(Box::new(|tx| {
                tx.send(AppEvent::SkipNextWorldWritableScan);
            }));
        }
        if let (Some(approval), Some(approvals_reviewer), Some(permission_profile)) =
            (approval, approvals_reviewer, permission_profile.clone())
        {
        ...
        if let (Some(approval), Some(approvals_reviewer), Some(permission_profile)) =
            (approval, approvals_reviewer, permission_profile)
        {
