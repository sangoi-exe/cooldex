...
mod tests {
    ...
    #[tokio::test]
    async fn streamed_handoff_with_commands_and_paths_matches_full_render() {
        let full = concat!(
            "Feito.\n\n",
            "**Baseline limpo**\n\n",
            ...
            "- Mantive so o fix minimo em `codex-rs/tui/src/tui.rs`.\n\n",
            "**Validacao**\n\n",
            "- `cd codex-rs && just fmt`\n",
            "- `bash ./scripts/cargo-guard.sh cargo check -p codex-tui`\n",
            "- `bash ./scripts/cargo-guard.sh cargo check -p codex-tui --tests`\n",
            "- `bash ./scripts/cargo-guard.sh cargo test -p codex-tui --no-run`\n",
            "- `bash ./scripts/cargo-guard.sh cargo test -p codex-tui controller_loose_vs_tight_with_commit_ticks_matches_full -- --nocapture`\n",
            "- `bash ./scripts/cargo-guard.sh cargo test -p codex-tui controller_live_owner_file_refs_match_full_render -- --nocapture`\n",
            "- `bash ./scripts/cargo-guard.sh cargo test -p codex-tui streamed_generated_render_stress_message_matches_one_shot_render_through_tui_flush_path -- --nocapture`\n",
            "- `bash ./scripts/cargo-guard.sh cargo test -p codex-tui streamed_dense_numbered_file_refs_match_full_render_in_vt100_history -- --nocapture`\n\n",
            "**Estado atual**\n\n",
            "- Repo principal ainda sujo com:\n",
            "  - `codex-rs/tui/src/app.rs`\n",
            "  - `codex-rs/tui/src/chatwidget/tests.rs`\n",
            "  - `codex-rs/tui/src/chatwidget/snapshots/codex_tui__chatwidget__tests__streamed_generated_render_stress_message_matches_one_shot_render_through_tui_flush_path.snap`\n",
            "- `.sangoi` suja so com `.sangoi/planning/2026-04-17-live-vs-resume-render-divergence-followup-plan.md`\n",
        );

        let mut collector = MarkdownStreamCollector::new(/*width*/ Some(94), &test_cwd());
        let mut streamed = Vec::new();
        let mut index = 0usize;
        while index < full.len() {
            let mut end = (index + 3).min(full.len());
            while end < full.len() && !full.is_char_boundary(end) {
                end += 1;
            }
            let delta = &full[index..end];
