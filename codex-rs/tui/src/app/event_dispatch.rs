//! Main app event dispatch for the TUI app.
//!
//! This module owns dispatch from `AppEvent` into app state while leaving the central run loop
//! and app construction in `app.rs`.
        ...
        match event {
            ...
            AppEvent::PersistRealtimeAudioDeviceSelection { kind, name } => {
                let builder = match kind {
                    RealtimeAudioDeviceKind::Microphone => self
                        .config_edits_builder()
                        .set_realtime_microphone(name.as_deref()),
                    RealtimeAudioDeviceKind::Speaker => self
                        .config_edits_builder()
            ...
            AppEvent::PersistPlanModeReasoningEffort(effort) => {
                ...
                if let Err(err) = self.config_edits_builder().with_edits([edit]).apply().await {
                    tracing::error!(
                        error = %err,
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let apply_result = self.config_edits_builder().with_edits([edit]).apply().await;
                match apply_result {
                    Ok(()) => {
