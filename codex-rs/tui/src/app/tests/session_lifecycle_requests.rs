    ...
    let proxy = tokio::spawn(async move {
        ...
        loop {
            ...
            match &frame {
                ...
                // Merge-safety anchor: on Windows, an unread-settings shutdown close is ConnectionAborted and must stay a benign recording-proxy disconnect.
                #[cfg(windows)]
                Err(tokio_tungstenite::tungstenite::Error::Io(error))
                    if error.kind() == std::io::ErrorKind::ConnectionAborted => break,
                _ => {}
            }
