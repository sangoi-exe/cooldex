...
const INSTANCE_CHILD_DISCONNECTED_MESSAGE: &str =
    "The instance-owned app-server stopped unexpectedly. Restart Codex to continue.";
...
pub(super) async fn reconnect(
    ...
    // Merge-safety anchor: InstanceChild belongs to one TUI process and cannot be reconnected,
    // restarted, or replaced by a daemon after its owned child disappears.
    if matches!(&target, AppServerTarget::InstanceChild) {
        color_eyre::eyre::bail!("{INSTANCE_CHILD_DISCONNECTED_MESSAGE}");
    }
...
impl App {
    ...
    pub(super) fn begin_reconnect(&mut self) -> bool {
        if matches!(&self.app_server_target, AppServerTarget::InstanceChild) {
            self.reconnect.offline = true;
            self.reconnect.failed = true;
            self.app_event_tx.send(AppEvent::FatalExitRequest(
                INSTANCE_CHILD_DISCONNECTED_MESSAGE.to_string(),
            ));
            return true;
        }
