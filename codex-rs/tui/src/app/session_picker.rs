...
impl App {
    pub(super) async fn open_resume_picker(
        ...
        // Merge-safety anchor: an in-app picker must reuse the active InstanceChild endpoint so
        // opening it cannot spawn a second child for the same TUI.
        ...
        let picker_instance_child_endpoint = app_server.instance_child_endpoint();
        ...
        let picker_app_server = tokio::spawn(async move {
            crate::start_app_server_for_picker(
                &picker_config,
                &picker_target,
                picker_instance_child_endpoint,
                picker_state_db,
