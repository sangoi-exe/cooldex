...
pub async fn load_config_layers_state(
    ...
    // Merge-safety anchor: the active user config path may be an explicit
    // override, not just `$CODEX_HOME/config.toml`; project-trust resolution
    // and home-doc loading must follow that same user layer canon.
    // Add a layer for the active user config path. This is either the explicit
    // path from LoaderOverrides or $CODEX_HOME/config.toml. When user config is
    // ignored, preserve layer metadata without reading config.toml.
    ...
    } else if explicit_user_config_path_provided {
        load_config_toml_for_explicit_user_layer(fs, &user_file, |config_toml| {
            ConfigLayerEntry::new(
                ConfigLayerSource::User {
                    file: user_file.clone(),
                },
...
async fn load_config_toml_for_explicit_user_layer(
    ...
    load_config_toml_for_required_layer(
        fs,
        &AbsolutePathBuf::from_absolute_path(toml_file)?,
        create_entry,
    )
    .await
}
