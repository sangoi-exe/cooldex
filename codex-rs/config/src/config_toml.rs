...
pub struct ConfigToml {
    ...
    // Merge-safety anchor: child-agent base instructions remain a config-owned
    // surface; keep this field aligned with spawn/resume config plumbing rather
    // than leaving it behind in generated schema followers.
    ...
    /// Controls recall payload verbosity. When unset/false, use a compact
    /// string-list payload. When true, use the full/debug payload.
    /// Keep recall runtime/docs/schema aligned with this default.
    pub recall_debug: Option<bool>,

    /// Explicit policy knobs for `manage_context` orchestration.
    // Merge-safety anchor: recall/manage_context config knobs are a local
    // runtime divergence; keep this owner synced with core runtime validation,
    // docs, and generated schema.
...
pub struct AgentsToml {
    ...
    /// Defaults to `true`.
    // Merge-safety anchor: legacy collab preemption is still a live config
    // knob in this workspace; keep the agents schema aligned with the runtime
    // gate in core.
