...
impl OpenAiModelsEndpoint {
    ...
    // Merge-safety anchor: remote model catalog fetches use provider-scoped auth
    // snapshots and surface runtime-load failures on request paths.
...
impl ModelsEndpointClient for OpenAiModelsEndpoint {
    ...
    async fn uses_codex_backend(&self) -> bool {
        let auth = match self.auth().await {
            Ok(auth) => auth,
            Err(error) => {
                tracing::warn!(%error, "failed to load provider auth while checking model backend");
                return false;
            }
        };
