...
pub enum ProviderAccountError {
    AccountRuntimeLoad(AccountRuntimeLoadError),
...
impl fmt::Display for ProviderAccountError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::AccountRuntimeLoad(error) => {
                write!(f, "failed to load provider account auth state: {error}")
            }
...
impl From<AccountRuntimeLoadError> for ProviderAccountError {
    fn from(error: AccountRuntimeLoadError) -> Self {
        Self::AccountRuntimeLoad(error)
    }
}
...
impl ModelProvider for ConfiguredModelProvider {
    ...
    // Merge-safety anchor: provider-scoped auth must preserve AccountManager
    // runtime-load failures instead of fabricating unauthenticated provider state.
    async fn auth(&self) -> codex_protocol::error::Result<Option<CodexAuth>> {
    ...
    fn account_state(&self) -> ProviderAccountResult {
        let account = if self.info.requires_openai_auth {
            match self.auth_manager.as_ref() {
                Some(auth_manager) => match auth_manager.auth_cached()? {
                    Some(auth) => {
                                ...
                                | CodexAuth::AgentIdentity(_) => {
                                    ...
                                    match (email, plan_type) {
                                        (Some(email), Some(plan_type)) => {
                                            ProviderAccount::Chatgpt { email, plan_type }
                                        }
                                        _ => {
                                            return Err(
