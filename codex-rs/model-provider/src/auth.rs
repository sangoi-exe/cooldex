// Merge-safety anchor: model-provider auth is derived from canonical request-auth owners; do not introduce parallel token ownership or hidden provider fallbacks.
