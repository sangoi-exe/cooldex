// Merge-safety anchor: spawn_agents_on_csv must keep max_concurrency as the sole public concurrency field so the legacy agent-job surface fails loudly on stale aliases.
