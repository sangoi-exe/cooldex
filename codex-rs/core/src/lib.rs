...
// Merge-safety anchor: `session_prefix` plus the `prompt_gc_*` modules remain
// active local runtime wiring for hidden child-session prompt_gc behavior; keep
// them compiled while integrating compatible upstream helpers like `prompt_debug`.
mod session_prefix;
