...
impl Session {
    ...
    ) -> Option<Option<codex_protocol::protocol::SessionAgentTask>> {
    ...
    pub(super) async fn restore_persisted_agent_task(&self, rollout_items: &[RolloutItem]) {
        ...
        match agent_task_update {
            Some(agent_task) => {
                if let Some(registered_task) = self
                    .services
                    .agent_identity_manager
                    .restore_task_for_current_identity(agent_task.clone())
                    .await
                {
                    let mut state = self.state.lock().await;
                    state.set_agent_task(registered_task);
                } else {
                    debug!(
    ...
    pub(crate) async fn authorization_header_for_current_agent_task(
        ...
        let Some(agent_task) = self.cached_agent_task_for_current_binding().await else {
            return Ok(None);
        };
        ...
        let authorization_header_value = agent_task.authorization_header(&record)?;
        debug!(
            agent_runtime_id = %agent_task.agent_runtime_id,
            task_id = %agent_task.task_id,
            "using agent assertion authorization for current task request"
        );
        Ok(Some(authorization_header_value))
    }
