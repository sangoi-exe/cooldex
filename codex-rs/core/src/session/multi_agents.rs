...
// Merge-safety anchor: V2 usage guidance keeps known-target fan-in on completion-body-redacted
// conditions and reserves generic timed waits for mailbox/steer activity rather than recreating V1 polling.
const DEFAULT_MULTI_AGENT_V2_WAIT_AGENT_USAGE_HINT_TEXT: &str = "When calling `wait_agent`, prefer longer waits (minutes) for generic mailbox/steer activity to avoid busy polling. Use `{ targets, return_when: \"all_final\", disable_timeout: true }` for self-contained known-target fan-in without intermediate mailbox handling, `{ targets, return_when: \"any_final\", disable_timeout: true }` for incremental completion, and a generic timed `{ timeout_ms }` wait only for mailbox/steer activity.";
