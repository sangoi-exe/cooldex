        ...
        match self {
            ...
            Self::ActiveTurnNotSteerable { turn_kind } => {
                let turn_kind_label = match turn_kind {
                    ...
                    NonSteerableTurnKind::Sanitize => "sanitize",
                };
                ErrorEvent {
...
impl Codex {
    ...
    async fn spawn_internal(args: CodexSpawnArgs) -> CodexResult<CodexSpawnOk> {
        ...
        let account_plan_type = auth_manager
            .auth_cached()
            .map(|auth| auth.and_then(|auth| auth.account_plan_type()))
            .unwrap_or_else(|err| {
                warn!(error = %err, "failed to load cached auth while deriving service tier");
                None
            });
...
impl Session {
    ...
    async fn local_rollout_recorder(&self) -> Option<RolloutRecorder> {
        self.services.rollout.lock().await.clone()
    }
    ...
    pub(crate) async fn flush_rollout(&self) -> std::io::Result<()> {
        ...
        } else if let Some(rollout) = self.local_rollout_recorder().await {
            rollout.flush().await
        } else {
            Ok(())
        }
    ...
    pub(crate) async fn request_permissions_for_cwd(
        ...
        match turn_context.as_ref().approval_policy.value() {
            AskForApproval::Never => {
                return Err(
                    "request_permissions is disabled by approval_policy = \"never\"".to_string(),
                );
            }
            ...
            {
                return Err(
                    "request_permissions is disabled by granular.request_permissions = false"
                        .to_string(),
                );
            }
        ...
        // Merge-safety anchor: request_permissions Auto-review must preserve the
        // local Result failure channel; only explicit user/Auto-review denials may
        // serialize as an empty successful permission response.
        if crate::guardian::routes_approval_to_guardian(turn_context.as_ref()) {
            ...
            let review_rx = crate::guardian::spawn_approval_request_review_detailed(
                Arc::clone(self),
            ...
            let decision = tokio::select! {
                biased;
                _ = cancellation_token.cancelled() => crate::guardian::GuardianReviewDecision::Aborted,
                decision = review_rx => decision.unwrap_or_else(|_| {
                    crate::guardian::GuardianReviewDecision::Failed(
                        "automatic approval review channel closed before request_permissions completed"
                            .to_string(),
                    )
                }),
            };
            let response = match decision {
                crate::guardian::GuardianReviewDecision::Approved => RequestPermissionsResponse {
                    permissions: requested_permissions.clone(),
                    scope: PermissionGrantScope::Turn,
                    strict_auto_review: false,
                },
                crate::guardian::GuardianReviewDecision::Denied => RequestPermissionsResponse {
                    permissions: RequestPermissionProfile::default(),
                    scope: PermissionGrantScope::Turn,
                    strict_auto_review: false,
                },
                crate::guardian::GuardianReviewDecision::TimedOut => {
                    return Err(crate::guardian::guardian_timeout_message());
                }
                crate::guardian::GuardianReviewDecision::Aborted => {
                    return Err(
                        "automatic approval review was aborted before request_permissions completed"
                            .to_string(),
                    );
                }
                crate::guardian::GuardianReviewDecision::Failed(message) => return Err(message),
            };
        ...
        tokio::select! {
            ...
            _ = cancellation_token.cancelled() => {
                ...
                Err("request_permissions was cancelled before a response arrived".to_string())
            }
            response = rx_response => response.unwrap_or_else(|_| {
                Err("request_permissions response channel closed before a response arrived".to_string())
            }),
        }
    }
    ...
    pub async fn notify_request_permissions_response(
        ...
        match entry {
            Some(entry) => {
                ...
                .await;
                entry.tx_response.send(Ok(response)).ok();
            }
            None => {
    ...
    pub async fn notify_request_permissions_failure(&self, call_id: &str, message: String) {
        let turn_state = {
            let active = self.active_turn.lock().await;
            active
                .as_ref()
        ...
        let entry = match turn_state {
            Some(turn_state) => {
                let mut turn_state = turn_state.lock().await;
                turn_state.remove_pending_request_permissions(call_id)
            }
            None => None,
        };
        match entry {
            Some(entry) => {
                entry.tx_response.send(Err(message)).ok();
            }
            None => {
                warn!("No pending request_permissions found for call_id: {call_id}");
            }
    ...
    pub(crate) async fn persist_prompt_gc_replacement_history(
        ...
        let mut rollout_items = vec![RolloutItem::Compacted(CompactedItem {
            ...
            prompt_gc: Some(PromptGcCompactionMetadata {
                ...
                error_message: None,
                applied_unit_count: Some(applied_unit_count),
            }),
        })];
    ...
    pub(crate) async fn persist_rollout_items(&self, items: &[RolloutItem]) {
        ...
        if let Some(rollout) = self.local_rollout_recorder().await
            && let Err(e) = rollout.record_items(items).await
        {
            // Merge-safety anchor: recall/manage_context/prompt_gc still use
            // the local rollout recorder in no-LiveThread sessions; keep this
            // fallback aligned with durable thread-store persistence above.
            error!("failed to record rollout items: {e:#}");
        }
    }
    ...
    pub(crate) async fn update_rate_limits_with_visibility(
        &self,
        turn_context: &TurnContext,
        new_rate_limits: RateLimitSnapshot,
    ...
    pub async fn steer_input(
        ...
        match active_turn.tasks.first().map(|(_, task)| task.kind) {
            ...
            Some(crate::state::TaskKind::Sanitize) => {
                return Err(SteerInputError::ActiveTurnNotSteerable {
                    turn_kind: NonSteerableTurnKind::Sanitize,
                });
            }
    ...
    pub(crate) async fn current_rollout_path(&self) -> anyhow::Result<Option<PathBuf>> {
        if let Some(live_thread) = self.live_thread() {
            ...
            Ok(self
                .local_rollout_recorder()
                .await
                .map(|rollout| rollout.rollout_path().to_path_buf()))
        }
    }
