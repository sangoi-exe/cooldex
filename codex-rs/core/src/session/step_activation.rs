...
impl Session {
    ...
    pub(super) async fn apply_turn_settings(
        ...
        // Merge-safety anchor: a settings update targets the exact live task,
        // never a later task reusing its turn context or ID.
        ...
        // A later task may reuse the same context and turn ID. `task_done` is
        ...
        if !Arc::ptr_eq(&task.task_done, &task_done)
