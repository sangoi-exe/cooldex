...
// Merge-safety anchor: recovery reconstruction fixtures retain local post-compaction recovery
// alongside upstream Guardian, retained-context, and token-checkpoint state.
...
async fn record_initial_history_restores_pending_post_compact_recovery() {
    ...
    let raw_items = session
        .clone_history()
        .await
        .raw_items()
        .cloned()
        .collect::<Vec<_>>();
    assert_eq!(strip_metadata_from_items(&raw_items), replacement_history);
    assert_eq!(
        session
            .state
            .lock()
