...
pub(crate) async fn prepare_agent_spawn_config(
    ...
    prepare_agent_spawn_config_with_service_tier(session, step_context, options, None).await
}

/// Resolves child settings for V2 callers that expose a public service-tier override.
pub(crate) async fn prepare_agent_spawn_config_with_service_tier(
    session: &Session,
    step_context: &StepContext,
    options: SpawnConfigOptions<'_>,
    ...
    let is_v2_full_history_fork =
        options.version == SpawnConfigVersion::V2 && options.full_history_fork;
    // Merge-safety anchor: V2 full-history forks apply the captured parent identity atomically;
    // only non-full-history children resolve child defaults, V2 instruction snapshots, and tiers.
    if !is_v2_full_history_fork {
        ...
        apply_requested_spawn_agent_service_tier(session, &mut config, requested_service_tier)
            .await?;
        if options.version == SpawnConfigVersion::V2 {
...
pub(crate) async fn apply_spawn_agent_service_tier(
    ...
    apply_requested_spawn_agent_service_tier(session, config, None).await
}

async fn apply_requested_spawn_agent_service_tier(
    session: &Session,
    config: &mut Config,
    requested_service_tier: Option<&str>,
) -> Result<(), String> {
    ...
    if requested_service_tier.is_some() && !model_info.supports_service_tier(&service_tier) {
        ...
        return Err(format!(
            "Service tier `{service_tier}` is not supported for model `{model}`. Supported service tiers: {supported_service_tiers}"
        ));
    }
...
async fn apply_spawn_agent_base_instructions(
    session: &Session,
    config: &mut Config,
) -> Result<(), String> {
