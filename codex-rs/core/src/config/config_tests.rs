...
async fn project_root_glob_none_compiles_to_filesystem_pattern_entry() -> std::io::Result<()> {
    ...
    assert_eq!(file_system_sandbox_policy.glob_scan_max_depth, Some(2));
    ...
    let servers = BTreeMap::from([(
        ...
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
                ...
                env: None,
                env_vars: vec![
                    McpServerEnvVar::Name("ALPHA".to_string()),
                    McpServerEnvVar::Name("BETA".to_string()),
                ],
                cwd: None,
            },
            experimental_environment: None,
...
async fn replace_mcp_servers_serializes_sourced_env_vars() -> anyhow::Result<()> {
    ...
    assert!(
        serialized.contains(
            r#"env_vars = ["LEGACY", { name = "LOCAL_TOKEN", source = "local" }, { name = "REMOTE_TOKEN", source = "remote" }]"#
        ),
...
#[tokio::test]
async fn feature_requirements_accept_auto_review_as_guardian_approval_compat_key()
-> std::io::Result<()> {
    let codex_home = TempDir::new()?;
