...
#[tokio::test]
async fn cli_override_subagent_instructions_file_sets_subagent_base_instructions()
-> std::io::Result<()> {
    let tmp = tempdir()?;
    let codex_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&codex_home).await?;
    ...
    let instructions_path = tmp.path().join("subagent.md");
    tokio::fs::write(&instructions_path, "cli subagent instructions").await?;

    let cli_overrides = vec![(
        "subagent_instructions_file".to_string(),
        TomlValue::String(instructions_path.to_string_lossy().to_string()),
    )];
    ...
    assert_eq!(
        config.subagent_base_instructions.as_deref(),
        Some("cli subagent instructions")
    );
