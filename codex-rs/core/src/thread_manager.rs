...
struct ForkThreadInitialHistoryRequest {
...
impl ThreadManager {
    ...
    pub async fn fork_thread_with_auth_manager(
        ...
        self.fork_thread_with_initial_history(ForkThreadInitialHistoryRequest {
            snapshot,
            config,
            thread_store,
            history,
    ...
    #[allow(clippy::too_many_arguments)]
    // Merge-safety anchor: app-server history forks must preserve the
    // request-derived AuthManager when a fork derives config/auth from a target
    // thread instead of the process startup owner.
    pub async fn fork_thread_from_history_with_auth_manager<S>(
        &self,
        snapshot: S,
        config: Config,
        thread_store: Arc<dyn ThreadStore>,
    ...
    async fn fork_thread_with_initial_history(
        &self,
        request: ForkThreadInitialHistoryRequest,
    ) -> CodexResult<NewThread> {
        let ForkThreadInitialHistoryRequest {
            snapshot,
            config,
            thread_store,
            history,
