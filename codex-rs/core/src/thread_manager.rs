...
impl ThreadManager {
    ...
    pub async fn resume_thread_with_history(
        ...
        let (session_source, thread_source) = initial_history
            .get_resumed_session_sources()
            .unwrap_or_else(|| (self.state.session_source.clone(), None));
        // Merge-safety anchor: a resumed V2 root restores persisted metadata
        // before startup, and restoration failures must propagate.
...
impl ThreadManagerState {
    ...
    // Merge-safety anchor: persisted thread settings snapshots pass through
    // ThreadManager with the established not-found and fatal-error mapping.
