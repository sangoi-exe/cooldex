...
impl Session {
    ...
    pub(crate) async fn start_task<T: SessionTask>(
        ...
        let supports_prompt_gc = task.supports_prompt_gc();
        ...
        let (turn_state, prompt_gc_sidecar) = {
            ...
            let prompt_gc_sidecar = if should_enable_prompt_gc {
                Some(turn.ensure_prompt_gc_sidecar())
            } else {
                None
            };
            (Arc::clone(&turn.turn_state), prompt_gc_sidecar)
        };
        {
        ...
        if let Some(sidecar) = prompt_gc_sidecar {
            ...
            // invariants.
            {
                ...
                sidecar.bind_turn(turn_context.sub_id.clone());
            }
        }
