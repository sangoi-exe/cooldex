...
fn wait_output_schema() -> JsonValue {
    json!({
        ...
        "properties": {
            "agents": {
                "type": "object",
                "description": "Current runtime state keyed by agent id for every requested agent when the wait returns.",
                    ...
                    "properties": {
                        ...
                        "lastActivity": {
                            ...
                            "properties": {
                                "kind": {
                                    "type": "string",
                                    "description": "Latest visible activity kind for the agent."
                                },
                                "summary": {
                                    "type": "string",
                                    "description": "Short human-facing summary of the latest activity."
                                },
                                "occurredAt": {
                                    "type": "number",
                                    "description": "Unix timestamp in seconds for the latest activity."
                                }
                            },
                            "required": ["kind", "summary", "occurredAt"],
                            "additionalProperties": false
                        }
                    },
                    "required": ["status", "lastActivity"],
                    "additionalProperties": false
                }
            },
            "timed_out": {
                "type": "boolean",
                "description": "Whether the wait call returned before the requested wait condition was met."
            }
        },
...
fn create_close_agent_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        ...
        description: "Close an agent and any open descendants when they are no longer needed, and return the target agent's previous status before shutdown was requested. When `agents.allow_running_subagent_preemption` is false, close_agent rejects active sub-agents.".to_string(),
        strict: false,
        defer_loading: None,
        parameters: JsonSchema::Object {
            properties,
