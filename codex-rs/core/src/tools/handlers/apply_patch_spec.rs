...
pub fn create_apply_patch_freeform_tool(include_environment_id: bool) -> ToolSpec {
    ...
    ToolSpec::Freeform(FreeformTool {
        ...
        description: "The `apply_patch` tool can be used to edit files. Each file path may appear in only one patch operation; combine multiple edits to one file in a single `Update File` operation or use separate calls. This is a FREEFORM tool, so do not wrap the patch in JSON.".to_string(),
        defer_loading: None,
