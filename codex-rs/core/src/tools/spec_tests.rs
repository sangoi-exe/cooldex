// Merge-safety anchor: tool-spec tests must stay aligned with the workspace-local
// planner overlay that reinjects `manage_context`/`recall` on top of the shared
// upstream tool-registry plan.
