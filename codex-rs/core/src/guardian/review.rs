// Merge-safety anchor: guardian approval review is a permission boundary; preserve local approval, sandbox, and operator-surface invariants when upstream review flow changes.

use std::sync::Arc;

use codex_analytics::GuardianApprovalRequestSource;
use codex_analytics::GuardianReviewAnalyticsResult;
use codex_analytics::GuardianReviewDecision as AnalyticsGuardianReviewDecision;
...
impl GuardianReviewDecision {
    fn into_review_decision(self) -> ReviewDecision {
        match self {
            Self::Approved => ReviewDecision::Approved,
            Self::Denied | Self::Failed(_) => ReviewDecision::Denied,
            Self::TimedOut => ReviewDecision::TimedOut,
            Self::Aborted => ReviewDecision::Abort,
        }
    }
}
...
async fn run_guardian_review(
        ...
        .is_some_and(CancellationToken::is_cancelled)
    {
        track_guardian_review(
            ...
            GuardianReviewAnalyticsResult {
                decision: AnalyticsGuardianReviewDecision::Aborted,
        ...
        return GuardianReviewDecision::Aborted;
    }
    ...
    let (assessment, failure_message, count_denial_for_circuit_breaker) = match outcome {
        GuardianReviewOutcome::Completed(assessment) => {
            ...
            track_guardian_review(
                ...
                GuardianReviewAnalyticsResult {
                    decision: if approved {
                        AnalyticsGuardianReviewDecision::Approved
                    } else {
                        AnalyticsGuardianReviewDecision::Denied
                    },
            ...
            (assessment, None, count_denial_for_circuit_breaker)
        }
        GuardianReviewOutcome::Error(error) => match error {
            GuardianReviewError::Timeout => {
                ...
                track_guardian_review(
                    ...
                    &review_tracking,
                    GuardianReviewAnalyticsResult {
                        decision: AnalyticsGuardianReviewDecision::Denied,
                ...
                session
                    ...
                    .await;
                record_guardian_non_denial(&session, &assessment_turn_id).await;
                return GuardianReviewDecision::TimedOut;
            }
    ...
    if approved {
        ...
        GuardianReviewDecision::Failed(message)
    } else {
        GuardianReviewDecision::Denied
    }
}
async fn review_approval_request_detailed(
    session: &Arc<Session>,
    turn: &Arc<TurnContext>,
    review_id: String,
    request: GuardianApprovalRequest,
...
pub(crate) fn spawn_approval_request_review_detailed(
    session: Arc<Session>,
    turn: Arc<TurnContext>,
    review_id: String,
    request: GuardianApprovalRequest,
    retry_reason: Option<String>,
    external_cancel: Option<CancellationToken>,
) -> oneshot::Receiver<GuardianReviewDecision> {
    ...
    std::thread::spawn(move || {
        let Ok(runtime) = tokio::runtime::Builder::new_current_thread()
            ...
            let _ = tx.send(GuardianReviewDecision::Failed(
                "failed to initialize automatic approval review runtime".to_string(),
            ));
            return;
        };
...
pub(crate) async fn review_approval_request(
    ...
    review_approval_request_detailed(session, turn, review_id, request, retry_reason)
        .await
