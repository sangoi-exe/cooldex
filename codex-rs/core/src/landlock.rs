...
/// helper is a separate executable. We pass the canonical permission profile as
/// JSON and let the helper derive the runtime filesystem/network policies.
/// Merge-safety anchor: preserve `use_legacy_landlock` and proxy-network
/// allowances through the shared codex-sandboxing helpers while the workspace
/// still relies on the legacy Landlock override.
