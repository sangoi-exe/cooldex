...
pub(super) async fn migration_scenario() -> Result<Vec<responses::ResponsesRequest>> {
    ...
    // Merge-safety anchor: a persisted `window_id` alone does not promote a pre-marker checkpoint out of Legacy mode.
    // Pre-marker wire format, including an ordinary instruction recorded after the checkpoint.
