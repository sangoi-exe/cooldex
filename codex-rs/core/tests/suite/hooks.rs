...
async fn blocked_user_prompt_submit_persists_additional_context_for_next_turn() -> Result<()> {
    ...
    // Merge-safety anchor: a rejected UserPromptSubmit hook must retain its typed durable
    // admission outcome while preserving hook context for the next accepted turn.
    let admission = timeout(
        Duration::from_secs(5),
        test.codex
            .submit_user_input_and_wait_for_persisted_admission(TurnInputRequest::new(
                TurnInput::UserInput {
                    content: vec![UserInput::Text {
                        text: "blocked first prompt".to_string(),
                        text_elements: Vec::new(),
                    }],
                    client_id: Some("blocked-user-prompt-client-id".to_string()),
                },
            )),
    )
    .await
    .expect("blocked user prompt durable admission should resolve promptly");
    match admission {
        Err(UserMessageAdmissionError::RejectedByHook) => {}
        other => panic!("expected rejected durable admission, got {other:?}"),
    }
    wait_for_event(&test.codex, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
