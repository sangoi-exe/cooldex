...
async fn token_budget_context_uses_new_window_after_compaction(
    ...
    assert!(
        ...
        "token-budget handoff should receive admitted history before compaction"
    );
...
async fn token_budget_mid_turn_auto_compaction_resets_before_active_follow_up(
    ...
    assert!(
        !requests.iter().any(
            |request| request.body_contains_message_text_outside_handoff(
    ...
    assert!(
        !requests.iter().any(|request| request
            .body_contains_message_text_outside_handoff(AUTO_COMPACT_FALLBACK_PROMPT)),
