...
// Merge-safety anchor: an explicitly selected multi-agent policy survives an active model switch; destination defaults must not reactivate proactive delegation.
#[test_case(Some(ReasoningEffort::Ultra); "selected effort")]
#[test_case(None; "captured model default effort")]
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn active_model_switch_keeps_explicit_multi_agent_policy_across_efforts(
    ...
    let explicit_text = "Any earlier instruction enabling proactive multi-agent delegation no longer applies. Do not spawn sub-agents unless the user or applicable AGENTS.md/skill instructions explicitly ask for sub-agents, delegation, or parallel agent work.";
    let proactive_text = "Proactive multi-agent delegation is active.";
    for request in &requests {
        assert!(request.body_contains_text(explicit_text));
        assert!(!request.body_contains_text(proactive_text));
    }
...
// Merge-safety anchor: every model's generated `spawn_agent` description retains full-history identity restrictions across an active model switch.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn tool_messages_follow_mid_turn_model_changes() -> Result<()> {
    skip_if_no_network!(Ok(()));

    const FULL_HISTORY_IDENTITY_GUIDANCE: &str = "Full-history forks (`fork_turns` omitted or set to `all`) inherit the parent agent type, provider, model, reasoning effort, reasoning summary, base instructions, developer instructions, and service tier as one identity. They reject `agent_type`, `model`, `reasoning_effort`, and `service_tier` overrides. Use `fork_turns=\"none\"` or a positive integer when an identity override is needed.";
            ...
            .map(|model| json!({
                ...
                "multi_agent_messages": MULTI_AGENT_TOOLS
                    .map(|name| (name.to_string(), json!({
                        "description": if name == "spawn_agent" {
                            format!("{name} description for {model}.\n{FULL_HISTORY_IDENTITY_GUIDANCE}")
                        } else {
                            format!("{name} description for {model}.")
                        },
