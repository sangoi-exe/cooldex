...
async fn compaction_checkpoints_settings_changed_during_its_model_request() -> Result<()> {
    ...
    let (server, _completions) = start_streaming_sse_server(vec![
        ...
        // Merge-safety anchor: local streaming compaction prepares a hidden operation-local handoff response before the gated compaction response under test.
        vec![StreamingSseChunk {
            gate: None,
            body: responses::sse(vec![
                responses::ev_assistant_message("pre-compact-handoff", "continue after compaction"),
    ...
    let request: serde_json::Value = serde_json::from_slice(&server.requests().await[2])?;
