...
fn assert_compacted_user_metadata(path: &Path, retained_text: &str) -> Result<()> {
    let (replacement_history, recovery_marker) = fs::read_to_string(path)?
        .lines()
        .filter(|line| !line.trim().is_empty())
        .map(codex_rollout::parse_rollout_line)
        .collect::<std::result::Result<Vec<_>, _>>()?
        ...
        .find_map(|line| match line.item {
            RolloutItem::Compacted(compacted) => {
                let replacement_history = compacted.replacement_history?;
                Some((replacement_history, compacted.post_compact_recovery))
            }
            _ => None,
        })
    ...
    let recovery_marker = recovery_marker.context("compacted recovery marker missing")?;
    let replacement_boundary_item_id = replacement_history
        .last()
        .and_then(|envelope| envelope.item.id())
        .context("compacted replacement history boundary item missing stable id")?;
    assert_eq!(
        recovery_marker.boundary_item_id,
        replacement_boundary_item_id.to_string(),
        "the durable recovery marker must bind the final replacement history item id"
    );
    Ok(())
}
...
// Merge-safety anchor: remote V2 checkpoints durably bind replacement-history ids before
// publishing live recovery and handoff overlays.
...
async fn remote_compact_v2_retains_metadata_from_resumed_history() -> Result<()> {
    ...
    assert!(
        handoff_request
            .body_json()
            .to_string()
            .contains(retained_text),
        "remote-v2 handoff should receive admitted history before durable replacement"
    );
...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    assert_eq!(
        follow_up
            ...
            .iter()
            .any(|text| text == "INJECTED_CLIENT_DEVELOPER"),
        enabled
    );
    requests
        .iter()
...
// Merge-safety anchor: remote V2 recovery order remains structural across successive compactions.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
