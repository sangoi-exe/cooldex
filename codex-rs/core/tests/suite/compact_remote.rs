...
async fn remote_compact_v2_retains_only_client_developer_messages_when_enabled(
    ...
    let retained_client_developer = follow_up
        .message_input_texts("developer")
        .iter()
        .any(|text| text == "INJECTED_CLIENT_DEVELOPER");
    assert_eq!(retained_client_developer, enabled);
    requests
        .iter()
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let compaction_items = follow_up_request.inputs_of_type("compaction");
    assert_eq!(
        compaction_items.len(),
        1,
        "expected follow-up request to use one compacted-history item"
    );
    assert_eq!(
        compaction_items[0]["encrypted_content"], "ENCRYPTED_COMPACTION_SUMMARY",
