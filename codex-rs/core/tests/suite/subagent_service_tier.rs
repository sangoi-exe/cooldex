//! Verifies that V2 children retain resolved service tiers while new children use the root tier.
...
#[test_case(Some("priority"), None; "disabling fast mode preserves active and idle V2 child work")]
...
async fn root_service_tier_change_preserves_existing_v2_subagent(
    ...
    assert_request_service_tier(&continued_child_request, initial_service_tier);
    ...
    wait_for_turn_complete(&child).await;
    assert_request_service_tier(&child_compaction_request, initial_service_tier);
    ...
    child
        ...
        .await?;
    wait_for_turn_complete(&child).await;
    assert_request_service_tier(&future_child_request, initial_service_tier);
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn evicted_v2_subagent_preserves_service_tier_after_reload() -> Result<()> {
    let server = start_mock_server().await;
    let mut builder = test_codex()
        .with_model("gpt-5.6-sol")
        .with_config(|config| {
    ...
    assert_eq!(
        reloaded_thread
            ...
            .as_deref(),
        Some("priority"),
        "reload must preserve the V2 child's resolved identity"
    );
