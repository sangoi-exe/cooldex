...
async fn legacy_checkpoint_recovers_root_excerpt_before_discarding_backup(
    ...
    // Merge-safety anchor: legacy source stays before the terminal checkpoint summary so
    // its post-compact recovery boundary ID remains unchanged during replay.
    let terminal_index = window
        .len()
        .checked_sub(1)
        .context("checkpoint terminal summary")?;
    match source_location {
        LegacyInstructionSource::GuardianHistory => {
            ...
            *content = vec![ContentItem::InputText {
            ...
            window.insert(terminal_index, shortened.into());
            checkpoint.guardian_history = Some(GuardianHistoryCheckpoint(vec![source]));
        }
        LegacyInstructionSource::ModelWindow => window.insert(terminal_index, source.into()),
