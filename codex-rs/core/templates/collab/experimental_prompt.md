...
* If you are awaiting sub-agents, prefer one `wait_agent(...)` call that matches the real intent:
  * use `wait_agent(..., return_when="any_final", disable_timeout=true)` when you want the next requested agent that is not already final to reach a final status; if every requested agent is already final, the wait may return immediately; if a requested agent is already or becomes errored, the explicit wait returns immediately so you can handle the failure
  * use `wait_agent(..., return_when="all_final", disable_timeout=true)` only when you truly need every requested agent to reach a final status; if a requested agent is already or becomes errored, the explicit wait returns immediately so you can handle the failure
