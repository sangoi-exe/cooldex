...
// Merge-safety anchor: spawn-agent turn_start items must preserve effective child
// profile/model/reasoning metadata so app-server notifications stay aligned with core rollout and
// TUI spawn labels.
#[tokio::test]
async fn turn_start_emits_spawn_agent_item_with_effective_profile_metadata_v2() -> Result<()> {
    skip_if_no_network!(Ok(()));

    const CHILD_PROMPT: &str = "child: do work";
    const PARENT_PROMPT: &str = "spawn a child and continue";
    const SPAWN_CALL_ID: &str = "spawn-call-1";
    const PROFILE_NAME: &str = "spawn-test";
    const PROFILE_MODEL: &str = "gpt-5.1";
    const PROFILE_REASONING_EFFORT: ReasoningEffort = ReasoningEffort::Low;
...
"#
    ...
    assert_eq!(
        ...
        ThreadItem::CollabAgentToolCall {
            ...
            model: Some(PROFILE_MODEL.to_string()),
            reasoning_effort: Some(PROFILE_REASONING_EFFORT),
    ...
    assert_eq!(profile, Some(PROFILE_NAME.to_string()));
...
#[tokio::test]
async fn turn_start_emits_spawn_agent_item_without_fabricated_reasoning_effort_v2() -> Result<()> {
    skip_if_no_network!(Ok(()));
