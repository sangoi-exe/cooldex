...

        {
            ...
            if index == 0 {
                ...
                let parent_items = parent[index * 2 + 1]["input"]
                    .as_array()
                    .expect("request input array");
                let checked_parent_items = parent_items
                    .iter()
                    .filter(|item| {
                        let is_historical_handoff = item["internal_chat_message_metadata_passthrough"]
                            ["content_item_kinds"]
                            == json!(["compaction.post_compact_handoff"]);
                        if is_historical_handoff {
                            assert_eq!(item["role"], "assistant");
                            let content = item["content"].as_array().expect("handoff content array");
                    ...
                    .collect::<Vec<_>>();
                let checked_parent_input = serde_json::to_string(&checked_parent_items)?;
                // Merge-safety anchor: filter only the tagged transient post-compaction handoff
                // from compacted-history checks; all other parent-input checks remain complete.
                // V2 keeps bounded user history while replacing old tool output with a checkpoint.
                assert!(checked_parent_input.contains(RESTRICTION));
                assert!(!checked_parent_input.contains(EVIDENCE));
                assert!(!checked_parent_items.iter().any(|item| {
                    item["type"] == "function_call_output" && item["call_id"] == "inspect-1"
                }));
                {
