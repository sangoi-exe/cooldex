...
async fn parent_response(
    ...
    // Merge-safety anchor: the transient pre-compaction handoff bypasses the parent-response sequence so the later compaction response remains the one guardian tests inspect.
    let events = if request["client_metadata"]["x-codex-turn-metadata"]
        .as_str()
