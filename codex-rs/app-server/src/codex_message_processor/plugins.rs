...
impl CodexMessageProcessor {
    ...
    async fn plugin_list_response(
        ...
        let auth = match self.auth_manager.auth().await {
            ...
            Err(err) => {
                warn!(
                    "failed to load auth for plugin list; continuing with local plugin catalog only: {err}"
                );
                None
            }
        };
        ...
        let request_auth = match self.auth_manager.chatgpt_request_auth().await {
            Ok(auth) => auth,
            Err(err) => {
                warn!(
                    "failed to load ChatGPT auth for featured plugin list; returning empty featured ids: {err}"
                );
                None
            }
        };
            ...
            .any(|marketplace| marketplace.name == OPENAI_CURATED_MARKETPLACE_NAME)
        {
            match plugins_manager
                .featured_plugin_ids_for_config(&config, request_auth.as_ref())
                .await
            {
    ...
    async fn plugin_read_response(
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                .await
                .map_err(|error| {
                    internal_error(format!("failed to load plugin app summaries: {error:#}"))
                })?;
            ...
            Err(remote_marketplace_name) => {
                ...
                let auth = self.auth_manager.auth().await.map_err(|err| {
                    internal_error(format!("failed to load auth for remote plugin read: {err}"))
                })?;
                let remote_plugin_service_config = RemotePluginServiceConfig {
                    chatgpt_base_url: config.chatgpt_base_url.clone(),
                };
    ...
    async fn plugin_install_response(
        ...
        let auth = match self.auth_manager.auth().await {
            ...
            Err(err) => {
                warn!(
                    "failed to load auth after plugin install; assuming installed plugin apps need no auth: {err}"
                );
                None
            }
        };
    ...
    async fn remote_plugin_install_response(
        ...
        let auth = self.auth_manager.auth().await.map_err(|err| {
            internal_error(format!(
                "failed to load auth for remote plugin install: {err}"
            ))
        })?;
        let remote_plugin_service_config = RemotePluginServiceConfig {
            chatgpt_base_url: config.chatgpt_base_url.clone(),
        ...
        let auth_snapshot =
            match connectors::load_connector_auth_snapshot(self.auth_manager.as_ref()).await {
                ...
                Err(err) => {
                    warn!(
                        plugin = plugin_id,
                        "failed to load connector auth after plugin install: {err:#}"
                    );
                    return Vec::new();
                }
            };
    ...
    async fn remote_plugin_uninstall_response(
        ...
        let auth = self.auth_manager.auth().await.map_err(|err| {
            internal_error(format!(
                "failed to load auth for remote plugin uninstall: {err}"
            ))
        })?;
        let remote_plugin_service_config = RemotePluginServiceConfig {
            chatgpt_base_url: config.chatgpt_base_url.clone(),
