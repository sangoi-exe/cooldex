// Merge-safety anchor: remote-control transport status is operator-visible app-server state; keep auth, state runtime, and websocket lifecycle aligned with local app-server customization.
