...
fn start_uninitialized(args: InProcessStartArgs) -> InProcessClientHandle {
    ...
    let runtime_handle = tokio::spawn(async move {
        ...
        // Merge-safety anchor: in-process app-server startup must reuse the
        // embedder-provided AuthManager so TUI/exec callers do not create a
        // second saved-account runtime owner inside the same process.
