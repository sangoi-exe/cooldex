...
pub(crate) async fn apply_bespoke_event_handling(
    ...
    match msg {
        ...
        // Merge-safety anchor: live warning notifications must emit the same
        // `ThreadItem::Warning` surface that replay/thread-read materialize,
        // and they must attach to a truthful turn owner instead of inventing ids.
        EventMsg::Warning(warning_event) => {
            if let ApiVersion::V2 = api_version {
                let turn_id = {
                    let state = thread_state.lock().await;
                    state.warning_turn_id()
                };
                let Some(turn_id) = turn_id else {
                    warn!(
                        thread_id = %conversation_id,
                        "dropping warning because no truthful turn is available"
                    );
                    return;
                };
                let notification = ItemCompletedNotification {
                    ...
                    item: ThreadItem::Warning {
                        id: format!("warning-{}", Uuid::now_v7()),
                        message: warning_event.message,
                    },
                };
                if let Some(analytics_events_client) = analytics_events_client.as_ref() {
                    analytics_events_client.track_notification(ServerNotification::ItemCompleted(
                        notification.clone(),
                    ));
                }
                outgoing
        ...
        EventMsg::CollabAgentSpawnEnd(end_event) => {
            ...
            let (receiver_thread_ids, agents_states) = match end_event.new_thread_id {
                Some(id) => {
                    ...
                    let received_status = collab_agent_state_with_identity(
        ...
        EventMsg::CollabWaitingBegin(begin_event) => {
            let item = collab_wait_begin_item(begin_event);
            let notification = ItemStartedNotification {
                thread_id: conversation_id.to_string(),
                turn_id: event_turn_id.clone(),
                item,
        ...
        EventMsg::CollabWaitingEnd(end_event) => {
            let item = collab_wait_end_item(end_event);
            let notification = ItemCompletedNotification {
                thread_id: conversation_id.to_string(),
                turn_id: event_turn_id.clone(),
                item,
        ...
        EventMsg::CollabCloseEnd(end_event) => {
            ...
            let agents_states = [(
                receiver_id.clone(),
                collab_agent_state_with_identity(
                    end_event.status,
                    end_event.receiver_agent_nickname.clone(),
                    end_event.receiver_agent_role.clone(),
                    end_event.receiver_agent_task_name.clone(),
...
fn collab_wait_begin_item(
    begin_event: codex_protocol::protocol::CollabWaitingBeginEvent,
) -> ThreadItem {
    ...
    ThreadItem::CollabAgentToolCall {
        ...
        reasoning_effort: None,
        agents_states: HashMap::new(),
        // Merge-safety anchor: live wait notifications must carry wait-state metadata so
        // any_final/all_final and timeout outcomes stay visible outside raw tool output.
        wait_state: Some(begin_event.wait_state.into()),
    }
}

fn collab_wait_end_item(end_event: codex_protocol::protocol::CollabWaitingEndEvent) -> ThreadItem {
    ...
    ThreadItem::CollabAgentToolCall {
        ...
        reasoning_effort: None,
        agents_states,
        wait_state: Some(end_event.wait_state.into()),
    }
}
...
fn collab_agent_state_with_identity(
...
mod tests {
    ...
    fn collab_resume_begin_maps_to_item_started_resume_agent() {
        ...
        let expected = ThreadItem::CollabAgentToolCall {
            ...
            receiver_agents: vec![V2CollabAgentRef {
    ...
    #[test]
    fn collab_wait_begin_maps_to_item_started_wait() {
        ...
        let item = collab_wait_begin_item(event.clone());
        let expected = ThreadItem::CollabAgentToolCall {
            id: event.call_id,
            tool: CollabAgentTool::Wait,
            status: V2CollabToolCallStatus::InProgress,
    ...
    #[test]
    fn collab_wait_end_maps_to_item_completed_wait() {
        let receiver_thread_id = ThreadId::new();
        ...
        let item = collab_wait_end_item(event.clone());
        let expected = ThreadItem::CollabAgentToolCall {
            id: event.call_id,
            tool: CollabAgentTool::Wait,
    ...
    #[tokio::test]
    async fn warning_events_emit_item_completed_notification_for_active_truthful_turn() -> Result<()>
    {
        let codex_home = TempDir::new()?;
        let config = load_default_config_for_test(&codex_home).await;
        let thread_manager = Arc::new(
        ...
        match msg {
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(payload)) => {
                assert_eq!(payload.thread_id, conversation_id.to_string());
                assert_eq!(payload.turn_id, "turn-active");
                let ThreadItem::Warning { id, message } = payload.item else {
                    bail!("expected warning item");
                };
                assert!(id.starts_with("warning-"));
                assert_eq!(message, "active warning");
            }
            other => bail!("unexpected message: {other:?}"),
        }
        assert!(rx.try_recv().is_err(), "no extra messages expected");
    ...
    #[tokio::test]
    async fn warning_events_emit_item_completed_notification_for_retained_truthful_turn()
    -> Result<()> {
        let codex_home = TempDir::new()?;
        let config = load_default_config_for_test(&codex_home).await;
        let thread_manager = Arc::new(
        ...
        apply_bespoke_event_handling(
            Event {
                id: "warning-event".to_string(),
                msg: EventMsg::Warning(codex_protocol::protocol::WarningEvent {
                    message: "retained warning".to_string(),
                }),
            },
            conversation_id,
            conversation,
        ...
        match msg {
            OutgoingMessage::AppServerNotification(ServerNotification::ItemCompleted(payload)) => {
                ...
                assert!(id.starts_with("warning-"));
                assert_eq!(message, "retained warning");
            }
            other => bail!("unexpected message: {other:?}"),
        }
        assert!(rx.try_recv().is_err(), "no extra messages expected");
    ...
    #[tokio::test]
    async fn warning_events_emit_item_completed_notification_after_mismatched_turn_complete()
    -> Result<()> {
        let codex_home = TempDir::new()?;
        let config = load_default_config_for_test(&codex_home).await;
        let thread_manager = Arc::new(
    ...
    async fn warning_events_without_truthful_turn_emit_no_notification() -> Result<()> {
        ...
        apply_bespoke_event_handling(
            Event {
                id: "warning-event".to_string(),
                msg: EventMsg::Warning(codex_protocol::protocol::WarningEvent {
                    message: "implicit warning".to_string(),
                }),
            },
            conversation_id,
            conversation,
        ...
        assert!(rx.try_recv().is_err(), "no warning notification expected");
        Ok(())
    }
