...
The same setting can be persisted in the active user config file via the top-level `sandbox_mode = "MODE"` key. When no override is in use, this defaults to `~/.codex/config.toml`, e.g. `sandbox_mode = "workspace-write"`.
