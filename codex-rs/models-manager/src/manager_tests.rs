    ...
    async fn uses_codex_backend(&self) -> bool {
        match self.auth_manager.as_ref() {
            Some(auth_manager) => match auth_manager.auth().await {
                Ok(auth) => auth.as_ref().is_some_and(CodexAuth::uses_codex_backend),
                Err(_) => false,
            },
            None => false,
        }
...
#[tokio::test]
async fn build_available_models_keeps_chatgpt_only_models_for_external_chatgpt_tokens() {
    let codex_home = tempdir().expect("temp dir");
    let auth = chatgpt_auth_tokens_for_tests(codex_home.path()).await;
    ...
    let manager = StaticModelsManager::new(
        Some(AuthManager::from_auth_for_testing(auth)),
        ModelsResponse {
            models: vec![chatgpt_only_model.clone(), api_model],
        },
    );

    let available = manager.build_available_models(manager.get_remote_models().await);
