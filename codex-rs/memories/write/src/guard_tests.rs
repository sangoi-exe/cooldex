...
fn snapshot(
    ...
    RateLimitSnapshot {
        ...
        limit_name: None,
        primary: primary_remaining_percent.map(window),
        secondary: secondary_remaining_percent.map(window),
        credits: None,
        plan_type: None,
...
fn window(remaining_percent: f64) -> RateLimitWindow {
    RateLimitWindow {
        remaining_percent,
        window_minutes: None,
        resets_at: None,
...
fn startup_check_uses_configured_remaining_threshold() {
    let snapshot = snapshot(
        /*primary_remaining_percent*/ Some(10.1),
        /*secondary_remaining_percent*/ Some(50.0),
    );
...
fn startup_check_skips_when_primary_or_secondary_is_too_low() {
    assert!(!snapshot_allows_startup(
        &snapshot(
            /*primary_remaining_percent*/ Some(24.9),
            /*secondary_remaining_percent*/ Some(90.0),
        ),
    ...
    assert!(!snapshot_allows_startup(
        &snapshot(
            ...
            /*secondary_remaining_percent*/ Some(24.9),
        ),
        /*min_remaining_percent*/ 25,
    ));
    assert!(snapshot_allows_startup(
        &snapshot(
            /*primary_remaining_percent*/ Some(25.1),
            /*secondary_remaining_percent*/ Some(25.1),
        ),
        /*min_remaining_percent*/ 25,
    ));
}
