...
fn snapshot_allows_startup(snapshot: &RateLimitSnapshot, min_remaining_percent: i64) -> bool {
    ...
    let min_remaining_percent = min_remaining_percent.clamp(0, 100) as f64;
    window_allows_startup(snapshot.primary.as_ref(), min_remaining_percent)
        && window_allows_startup(snapshot.secondary.as_ref(), min_remaining_percent)
}

fn window_allows_startup(window: Option<&RateLimitWindow>, min_remaining_percent: f64) -> bool {
    match window {
        Some(window) => window.remaining_percent >= min_remaining_percent,
        None => true,
    }
}
