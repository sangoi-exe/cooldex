...
impl RolloutRecorder {
    ...
    pub async fn persist_items_atomically(&self, items: &[RolloutItem]) -> std::io::Result<()> {
    ...
    pub async fn load_rollout_items(
        ...
        for line in text.lines() {
            ...
            if strip_legacy_ghost_snapshot_rollout_line(&mut line_value) {
...
impl RolloutWriterState {
    ...
    async fn add_items_atomically(&mut self, items: &[RolloutItem]) -> std::io::Result<()> {
    ...
    while let Some(cmd) = rx.recv().await {
        match cmd {
            ...
            RolloutCmd::AddItemsAtomically { items, ack } => {
                let result = state.add_items_atomically(items.as_slice()).await;
