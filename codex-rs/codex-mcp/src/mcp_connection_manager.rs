...
/// servers should run for the current caller. `fallback_cwd` is used only when
/// an executor-backed stdio server omits `cwd` and the executor API still
/// needs a concrete process working directory.
#[derive(Clone)]
