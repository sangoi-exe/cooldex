...
//! `codex-core`.
//!
//! Merge-safety anchor: this manager is the MCP mutation/tool-call owner; keep env-source routing, provenance, and cache semantics centralized here.
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        _auth: Option<&ChatGptRequestAuth>,
    ...
    /// Returns a future over a cloned client snapshot so callers can drop
    /// external manager guards before awaiting tool aggregation.
    pub fn list_all_tools_snapshot(
        &self,
    ) -> impl std::future::Future<Output = HashMap<String, ToolInfo>> + Send + 'static {
