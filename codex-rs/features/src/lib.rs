...
// Merge-safety anchor: merge the local Computer Use/multi-agent feature surface with
// upstream context management so the persisted recovery/retained/token history contract remains intact.
