...
pub enum Op {
    ...
    /// Fail a request_permissions tool call without laundering the failure as a denial.
    RequestPermissionsFailure {
        /// Call id for the in-flight request.
        id: String,
        /// Failure reason returned to the model/tool caller.
        message: String,
    },
...
impl Op {
    pub fn kind(&self) -> &'static str {
        match self {
            ...
            Self::StopBackgroundTerminals => "stop_background_terminals",
            ...
            Self::RequestPermissionsFailure { .. } => "request_permissions_failure",
        ...
        match self {
            ...
            InitialHistory::Resumed(resumed) => resumed.history.iter().rev().find_map(|item| {
                if let RolloutItem::TurnContext(turn_ctx) = item {
                    Some(turn_ctx.developer_instructions.clone())
                } else {
                    None
                }
            }),
...
pub struct SessionConfiguredEvent {
    ...
    ///
    /// Merge-safety anchor: `permission_profile` is canonical, while
    /// `sandbox_policy` remains the required compatible legacy projection for
    /// clients that have not moved to the profile contract.
    pub permission_profile: PermissionProfile,

    /// Legacy sandbox projection for clients that have not moved to
    /// `permission_profile`.
    pub sandbox_policy: SandboxPolicy,
...
impl<'de> Deserialize<'de> for SessionConfiguredEvent {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let sandbox_policy =
            permission_profile.to_compatible_legacy_sandbox_policy(wire.cwd.as_path());
...
mod tests {
    ...
    fn serialize_event() -> Result<()> {
        ...
        let cwd_abs = cwd.abs();
        let sandbox_policy =
            permission_profile.to_compatible_legacy_sandbox_policy(cwd_abs.as_path());
        let event = Event {
            id: "1234".to_string(),
