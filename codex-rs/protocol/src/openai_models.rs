...
// Merge-safety anchor: proactive model guidance remains policy-driven rather than tied to a model tier.
#[derive(Debug, Serialize, Deserialize, Clone, PartialEq, Eq, TS, JsonSchema)]
pub struct MultiAgentModeMessages {
    pub explicit: Option<String>,
    /// Model-specific instructions for the selected proactive policy. Missing or null uses the
    /// built-in proactive instructions; an empty string suppresses only this explanation.
    /// `hint_text` takes precedence.
