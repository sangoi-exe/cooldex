...
// Merge-safety anchor: persisted recovery/retained/token wire records and compacted checkpoints
// round-trip together; absent recovery omits, while token checkpoints remain nullable.
...
pub(super) enum RolloutItemWire<'a> {
    ...
    PostCompactRecoveryApplied {
        payload: Cow<'a, PostCompactRecoveryAppliedItem>,
    },
    TurnContext {
...
impl<'a> From<&'a RolloutItem> for RolloutItemWire<'a> {
    fn from(item: &'a RolloutItem) -> Self {
        match item {
            ...
            RolloutItem::PostCompactRecoveryApplied(payload) => Self::PostCompactRecoveryApplied {
                payload: Cow::Borrowed(payload),
            },
...
impl From<RolloutItemWire<'_>> for RolloutItem {
    fn from(item: RolloutItemWire<'_>) -> Self {
        match item {
            ...
            RolloutItemWire::PostCompactRecoveryApplied { payload } => {
                Self::PostCompactRecoveryApplied(payload.into_owned())
            }
...
pub(super) struct CompactedItemWire<'a> {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    post_compact_recovery: Option<Cow<'a, PostCompactRecoveryMarker>>,
    #[serde(default)]
