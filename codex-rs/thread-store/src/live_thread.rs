...
impl LiveThread {
    ...
    /// Returns the live local rollout recorder for legacy local-only callers.
    ///
    /// Remote stores do not expose rollout recorders, so they return `Ok(None)`.
    pub async fn local_rollout_recorder(&self) -> ThreadStoreResult<Option<RolloutRecorder>> {
        let Some(local_store) = self
            .thread_store
            .as_any()
            .downcast_ref::<LocalThreadStore>()
        ...
        local_store.live_recorder(self.thread_id).await.map(Some)
    }
}
