...
//! before the writer makes its second streaming pass. Retained checkpoints are reduced at their
//! acceptance-order rollback boundary rather than by reapplying prior rollbacks to later
//! checkpoints. This plan owns parsed-record visibility; the canonicalizer owns output bytes and
//! ordinals.
// Merge-safety anchor: Post-compaction recovery proofs are neutral to legacy rollback planning;
// retained answers use acceptance-order rollback semantics.
