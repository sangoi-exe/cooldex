...
async fn create_replacement_recorder(
    ...
    // Merge-safety anchor: revert copies the source Option verbatim so old metadata absence is
    // not rewritten as Resolve before the V2 restore boundary can reject it deliberately.
    .with_agent_usage_hint_binding(source_meta.agent_usage_hint_binding)
