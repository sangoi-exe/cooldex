...
# Merge-safety anchor: this local helper owns the CLI remote-exec sync/build/tunnel workflow for codex-exec-server, so upstream syncs must keep the operator path intact.
...
cleanup() {
  ...
  if [[ -n "${remote_exec_server_pid_path}" ]]; then
    ssh "${remote_host}" bash -s -- \
      "${remote_exec_server_pid_path}" \
      "${remote_exec_server_log_path}" <<'EOF' \
      >/dev/null 2>&1 || true
set -euo pipefail

remote_exec_server_pid_path="$1"
remote_exec_server_log_path="$2"

if [[ -f "${remote_exec_server_pid_path}" ]]; then
  kill "$(cat "${remote_exec_server_pid_path}")" >/dev/null 2>&1 || true
fi
rm -f "${remote_exec_server_pid_path}" "${remote_exec_server_log_path}"
EOF
  fi
...
if [[ ! -d "${REPO_ROOT}/codex-rs" ]]; then
  echo "workspace root ${REPO_ROOT} is missing codex-rs" >&2
  exit 1
fi
...
require_remote_tool() {
  ...
  if ! ssh "${remote_host}" "command -v ${tool} >/dev/null 2>&1"; then
    echo "remote ${tool} is required on ${remote_host}" >&2
    exit 1
  fi
}

require_remote_tool bash
require_remote_tool cargo
require_remote_tool rsync
...
remote_path="$1"
remote_exec_server_log_path="$2"
remote_exec_server_pid_path="$3"
remote_exec_server_start_timeout_seconds="$4"
remote_repo_root="$HOME/$remote_path"
remote_codex_rs="$remote_repo_root/codex-rs"
remote_target_dir="$remote_codex_rs/target"
remote_exec_server_bin="$remote_target_dir/debug/codex-exec-server"

cd "${remote_codex_rs}"
CARGO_TARGET_DIR="${remote_target_dir}" cargo build -p codex-exec-server --bin codex-exec-server

rm -f "${remote_exec_server_log_path}" "${remote_exec_server_pid_path}"
nohup "${remote_exec_server_bin}" --listen ws://127.0.0.1:0 \
