...
- Keep this aligned with the AccountManager-owned account summary projection surfaced through `auth_manager.account_manager().list_accounts()` and TUI account popups: `/accounts` renders only the summary fields exposed there.
