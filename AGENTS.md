...
0. Before each rung below, invoke it via `./scripts/cargo-guard.sh cargo ...`; the wrapper asks Cargo for the effective `target_directory`/`build_directory`, caps guarded Cargo parallelism at 4 jobs unless you explicitly request a lower count, rejects explicit higher `-j/--jobs` and `--config build.jobs` requests, and rejects path-style or include-based `--config` job-cap bypasses for guarded build-like commands, checks the 5 GiB floor across those directories, and runs `cargo clean` only when low free space violates that guardrail.
