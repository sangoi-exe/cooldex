...
- `prompt_gc` / `PromptGcSidecar`: automatic active-turn prompt GC via a hidden summary-only child-session flow, private runtime activity state, replacement-history persistence plus `TurnContext`-coupled replay/reconstruction, and the TUI prompt-GC indicator. Representative files: `codex-rs/core/src/prompt_gc_sidecar.rs`, `codex-rs/core/src/tools/handlers/prompt_gc.rs`, `codex-rs/core/prompt_gc_prompt.md`, `codex-rs/core/src/client_common.rs`, `codex-rs/core/src/client.rs`, `codex-rs/core/src/codex.rs`, `codex-rs/core/src/rollout/recorder.rs`, `codex-rs/core/src/codex/rollout_reconstruction.rs`, `codex-rs/core/src/tasks/mod.rs`, `codex-rs/tui/src/bottom_pane/mod.rs`, `codex-rs/tui/src/chatwidget.rs`, `codex-rs/tui/src/app.rs`.
